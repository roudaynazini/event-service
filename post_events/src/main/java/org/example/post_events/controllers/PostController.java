...
public class PostController {
    ...
    void onPartagerClicked(ActionEvent event) {
        ...
        if (selectedPost != null) {
            try {
                String message = selectedPost.getDescription();
                String url = "https://wa.me/?text=" + URLEncoder.encode(message, StandardCharsets.UTF_8);
                Desktop.getDesktop().browse(new URI(url));

                Alert confirmation = new Alert(Alert.AlertType.INFORMATION);
                confirmation.setTitle("Partage WhatsApp");
                confirmation.setHeaderText(null);
                confirmation.setContentText("Le post a été préparé pour être partagé sur WhatsApp.");
            ...
            alert.setTitle("Aucun post sélectionné");
            alert.setContentText("Veuillez sélectionner un post à partager.");
            alert.showAndWait();
        }
    ...
    private void afficherCommentairesDuPost(Post post) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/org/example/post_events/PostDetails.fxml"));
            Parent root = loader.load();
